...
public class YandexPredictorSyncApiImpl {

    private final String key;
    private final YandexPredictorApi api;

    public YandexPredictorSyncApiImpl() {
    ...
    public List<String> getLangs() {
        try {
            return api.getLangs(key).execute().body();
        } catch (IOException e) {
            throw new RuntimeException(e);
        }
    }

    public PredictResponse complete(String query, String lang, Integer limit) {
        try {
            return api.complete(key, query, lang, limit).execute().body();
        } catch (IOException e) {
            throw new RuntimeException(e);
        }
    }
